...
const IDENTIFIER = "com.justcallmebryan.pulso";
...
function printDone(tag, pushed) {
  console.log(`\n   ${paint(c.green + c.bold, "✓")}  released ${paint(c.bold, tag)}
   ${paint(c.dim, pushed ? "branch + tag pushed — GitHub will draft the macOS release" : "local only — push when ready")}
`);
}
...
if (tauriCurrent.identifier !== IDENTIFIER) {
  fail(
    `identifier is "${tauriCurrent.identifier}" but expected "${IDENTIFIER}". ` +
